...
if [[ -n "$SWAP_SETTINGS" ]]; then
	...
	# 1. Extract flags and their values: --flag value or --flag
	# This matches --[name] and optionally one following word that isn't a quote or space
	raw_flags=$(echo "$content" | rg -i -oP -- '--[a-zA-Z0-9_-]+(\s+[^"'"'"'\s]+)?' || true)
	if [[ -n "$raw_flags" ]]; then
		while IFS= read -r line; do
			# Normalize: remove leading dashes and split by space to get flag and value
			clean=$(echo "$line" | sed 's/^-*//')
			for word in $clean; do
				PRIORITY_TERMS+=("$(echo "$word" | tr '[:upper:]' '[:lower:]')")
			done
		done <<<"$raw_flags"
	fi

	# 2. Extract short flags: -np 2 -> matches -np
	raw_shorts=$(echo "$content" | rg -i -oP -- '-[a-zA-Z0-9_-]+' || true)
	if [[ -n "$raw_shorts" ]]; then
		while IFS= read -r line; do
			term=$(echo "$line" | sed 's/^-*//' | tr '[:upper:]' '[:lower:]')
			PRIORITY_TERMS+=("$term")
		done <<<"$raw_shorts"
	fi

	# 3. Extract quantization patterns: Q4_K_M, UD-Q5_K_XL, etc.
	# These often appear in filenames or as values
	quants=$(echo "$content" | rg -o 'Q[0-9]_[A-Z0-9_]+|UD-Q[0-9]_[A-Z0-9_]+' -i || true)
	if [[ -n "$quants" ]]; then
	...
	if echo "$lmsg" | grep -Ei "$KEYWORDS" >/dev/null; then
		...
		if [[ ${#PRIORITY_TERMS[@]} -gt 0 ]]; then
			for t in "${PRIORITY_TERMS[@]}"; do
				# Match the term flexibly: treat underscores/hyphens as interchangeable with spaces
				# e.g. "draft-mtp" matches "draft mtp", "draft-mtp", "draft_mtp"
				pattern=$(echo "$t" | sed 's/[-_]/[-_ ]/g')
				if [[ -n "$pattern" ]] && echo "$lmsg" | grep -Ei "$pattern" >/dev/null; then
					is_priority=1
					break
				fi
			done
